...
class SchemaClient:
    def __init__(self):
        self._client = httpx.AsyncClient(base_url=os.getenv("MCP_SERVER_URI") or "http://data-service:8001")
    ...
    async def get_schemas(self) -> List[Dict[str, Any]]:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.error(f"An error occurred while requesting schemas: {exc}")
                # Return the stale cache if available, otherwise an empty list
                return self._cache if self._cache is not None else []
